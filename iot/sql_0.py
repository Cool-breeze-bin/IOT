...
def insert(topics, content,con):
    cur = con.cursor()
    curr_time = datetime.datetime.now()
    data_time = datetime.datetime.strftime(curr_time, '%Y-%m-%d %H:%M:%S')
    cur.executemany("INSERT iot VALUE(%s,%s,%s)", [
                    (data_time, topics, content)])
    con.commit()
...
def main(topics, content):
    ...
    # topics = "Topics fangbinbin Pub"
    # content = "fangbinbin"
    insert(topics,content,con)
